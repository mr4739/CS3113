		...
		if(key == "data") {
			for(int y=0; y < mapHeight; y++) {
				...
				for(int x=0; x < mapWidth; x++) {
					...
					if(val > 0) {
						mapData[y][x] = val-1;
					} else  if (mapData[y][x] == 3452816845) {
						mapData[y][x] = 0;
					} 
				}
			}
...
void FlareMap::Load(const std::string fileName) {
	...
	while (std::getline(infile, line)) {
		...
		} else if(line == "[enemyLayer]") {
			ReadEntityData(infile);
		}
		else if (line == "[playerLayer]") {
			ReadEntityData(infile);
		}
	}
}

void FlareMap::printMapData() const {
	std::cout << "map info: " << mapHeight << " x " << mapWidth << std::endl;
	for (int y = 0; y < mapHeight; y++) {
		for (int x = 0; x < mapWidth; x++) {
